...
def prefetch_test(opt):
    ...
    for model_id in range(model_begin, model_end+1):
        ...
        if os.path.exists(os.path.join(model_path, 'best_predictions.json')):
            with open(os.path.join(model_path, 'best_predictions.json'), 'r') as f:
                output_hoi = json.load(f)
        else:
            if opt.load_model == '' or model_id > model_begin:
                opt.load_model = os.path.join(
                    model_path, 'model_' + str(model_id) + '.pth')
        ...
        if opt.test_with_eval:
            if 'hico' in opt.dataset:
                hoi_eval = hico(os.path.join(
                    opt.root_path, 'hico_det/annotations/test_hico.json'))
            elif 'vcoco' in opt.dataset:
                hoi_eval = vcoco(os.path.join(
                    opt.root_path, 'verbcoco/annotations/test_vcoco.json'))
            elif 'hoia' in opt.dataset:
                hoi_eval = hoia(os.path.join(
                    opt.root_path, 'hoia/annotations/test_hoia.json'))
    ...
    if opt.test_with_eval:
        print('best model id: {}, best map: {}'.format(
            map_dcit['best_id'], map_dcit['best_map']))
